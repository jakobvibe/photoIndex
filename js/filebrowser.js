...
function init() {
    ...
    imageFileTypes = ['jpg', 'JPG', 'jpeg', 'png', 'gif'];

    $.get(dataFile, function(data) {
        $(window).on('hashchange', function() {

            goto(window.location.hash, data);
            pswp.close();
        ...
        if (data.count) {
            var curOffset = getURLParameter('offset');
			var dataCnt = 1 * data.count;
			var nextOffset  = curOffset*1 + 1;
			
            if (curOffset &&  (dataCnt < nextOffset)) {
                $('.load-next').hide();
            }
        }
        loadPage(data)
    ...
    fileList.on('click', 'li.folders', function(e) {
        ...
        if (filemanager.hasClass('searching')) {
            ...
            breadcrumbsUrls = nextDir.split('/');
        }

        setUrlParameters({ 'path': nextDir });
        currentPath = nextDir;
    });
}
...
function doSearch(search) {
    clearPage();
    window.history.pushState("", "", '?view=search&q=' + search);
    $.getJSON(
        'getFullList.php', { 'q': search }
    ).done(function(data) {
        completeSearch(data);
    });
}
...
function render(renderData) {
    ...
    if (scannedFiles.length) {
        ...
        scannedFiles.forEach(function(f) {
            ...
            var fileSize = bytesToSize(f.size),
                ...
                largethumb = f.thumbnail_large ? f.thumbnail_large : url,
                artist = f.artist,
    ...
    if (false && filemanager.hasClass('searching')) {
        ...
        if (breadcrumbsUrls.length === 1 && typeof breadcrumbsUrls[0] != 'undefined') {
            breadcrumbsUrls = breadcrumbsUrls[0].split('/');
        }
        var breadcrumbPath = '?view=folder&path=';
        breadcrumbsUrls.forEach(function(u, i) {
            var name = typeof u != 'undefined' ? u.split('/') : '';

            if (i !== breadcrumbsUrls.length - 1) {
                breadcrumbPath += i > 0 ? '%2F' + u : u;
                if (i == 0) {
                    url += '<a href="/"><span class="folderName">' + name[name.length - 1] + '</span></a> <i class="arrow">/</i> ';
                } else {
                    url += '<a href="' + breadcrumbPath + '"><span class="folderName">' + name[name.length - 1] + '</span></a> <i class="arrow">/</i> ';
                }
            } else {
...
function loadInfo($file) {
    ...
    $info.find('.keywords.text').html(function() {
        ...
        for (var i = 0; i < keywordArr.length; i++) {
            result += '<a class="badge keyword" href="?view=search&q=' + keywordArr[i] + '">' + keywordArr[i] + '</a>';
        }
        return result;
    });
}
    ...
    var initPhotoSwipeFromDOM = function(gallerySelector) {
        ...
        var parseThumbnailElements = function(el) {
            ...
            for (var i = 0; i < numNodes; i++) {
                ...
                // create slide object
                item = {
                    src: el.getAttribute('data-largethumb'),
                    download: el.getAttribute('data-url'),
        ...
        var openPhotoSwipe = function(index, galleryElement, disableAnimation, fromURL) {
            ...
            options = {
                ...
                shareButtons: [
                    { id: 'link', label: 'Link', url: '{{raw_image_url}}' },
 ...
 *	@params object of key / value pairs. Set value to false to remove it.
 *			{'paramName' : 'paramValue', ...}
 */

function setUrlParameters(params, altPath) {
    var url = altPath ?
        window.location.protocol + "//" + window.location.host + '/' + altPath + window.location.search :
    ...
    if (paramIndex === -1) {
        ...
        baseUrl = url.substring(0, paramIndex);
        urlParameters = url.substring(paramIndex + 1);
    }

    urlParameters = $.deparam(urlParameters);
    parameters = $.extend({}, urlParameters, params);

    for (var cur in parameters) {
        if (parameters[cur] === false) {
            delete parameters[cur];
        }
    };

    var url = baseUrl + '?' + $.param(parameters);
